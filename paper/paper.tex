...
\textbf{Qwen3-Coder-30B} & \textbf{Few-shot (3) + Schema Link + Finetune} & \textbf{64\%} & \textbf{--} \\
\hline
\end{tabular}
...
Results indicate that ... (đoạn này viết tiếp phân tích cho tôi. Các lỗi thường gặp là model bịa ra tên cột/bảng, không biết các quy luật nhâp liệu dữ liệu ví dụ Los Angeles được nhập vào là LA, hay California -> CA... mặc dù đã có business rule và schema link hỗ trợ. Qua đó thấy được sự phức tạp của TPC-DS mà nghiên cứu [https://arxiv.org/pdf/2407.19517](https://arxiv.org/pdf/2407.19517) đã đề cập...  Gemini và OpenAI mặc định sẽ được sử dụng 2 kĩ thuật đi kèm để đảm bảo context để model hiểu về data warehousewarehouse)

\section{Conclusion and Future Work}
Our Voice-to-SQL pipeline demonstrates the feasibility of checking complex decision support queries via voice in Vietnamese. The combination of domain-specific ASRwarehousewarehouse (Gemini/Whisper) and schema-aware LLMs (Qwen3-Coder) yields promising results.
...
\bibitem{tpcds} TPC Council, "TPC-DS Benchmark," [https://www.tpc.org/tpcds/](https://www.tpc.org/tpcds/).
...
tiến hành bổ sung, hoàn thiện paper tôi đang viết và format tài liệu