...
The fine-tuning dataset consists of 190 manually curated queries (100 Easy, 90 Hard), ensuring coverage of complex SQL features like Window Functions and CTEs.
...
\begin{tabular}{@{}llcc@{}}
\toprule
\textbf{Model} & \textbf{Config} & \textbf{Acc} & \textbf{Exec} \\
\midrule
...
Fine-tuning delivered massive gains for smaller models (Qwen-4B improved from 17\% to 33%). For larger models (Qwen3-30B), while the absolute accuracy gain was marginal (1\%), fine-tuning was critical for \textbf{robustness}. It significantly reduced systematic errors such as hallucinated columns (down from 12\% to 4\%) and incorrect JOIN paths (down from 6\% to 2\%).
...
Despite achieving 64\% accuracy, our analysis reveals systematic failures that highlight the ``Data Warehouse Semantics Gap''---a problem less visible in simpler datasets:
...
TPC-DS's snowflake schema relies on standard data warehouse patterns. For instance:
\begin{itemize}
    \item Dimension tables (e.g., \texttt{date\_dim}, \texttt{customer\_dim}) should be joined via specific keys.
    \item Fact tables reference dimensions through Foreign Keys.
    \item Analytical queries typically aggregate fact tables after joining dimensions.
\end{itemize}

Models without warehouse-domain knowledge often miss these patterns, generating incorrect JOIN paths or redundant joins. This accounted for \textbf{6\%} of failures. Fine-tuning reduced this to 2\%, demonstrating knowledge internalization.

\paragraph{3. Context Window Limitations.}
Even with dynamic schema linking, providing all relevant table definitions often exceeds practical context windows. A typical TPC-DS query might reference 5--8 tables, each with 10--20 columns. Balancing schema completeness with context constraints remains challenging \cite{wang2024generalist}.

\subsubsection{Why Gemini and OpenAI Underperform (Despite Strong Overall Performance)}

Interestingly, Gemini and OpenAI (without fine-tuning) underperformed Qwen3-Coder-30B on TPC-DS despite being more powerful models on general benchmarks. Analysis suggests:
...
\begin{itemize}
    ...
    \item \textbf{Few-shot Learning Plateau}: Adding 5 examples achieved 59\% for Gemini, but further improvements required fine-tuning---which we did not apply to proprietary models.
