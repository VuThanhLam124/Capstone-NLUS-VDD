...
\begin{itemize}
    \item \textbf{ASR Evaluation}: We conducted a \textbf{custom evaluation on 800 audio samples} covering 8 regional Vietnamese accents and specialized SQL technical jargon, ensuring robust performance analysis beyond standard open datasets.
    \item \textbf{Text-to-SQL Dataset}: To rigorously evaluate our system, we constructed a custom benchmark suite of 190 Vietnamese queries distinct from the official TPC-DS set, manually curated and stratified by difficulty (Source: \texttt{research\_pipeline/datasets/*.jsonl} in our repository):
    \begin{itemize}
        ...
        \item \textbf{Test Set B: Hard (90 Queries)}: Targets multi-hop JOINs (3+ tables), complex window functions, CTEs, and nested subqueries. This set tests reasoning over dimensional hierarchies and implicit business logic.
    \end{itemize}
...
Fine-tuning delivered massive gains for smaller models (Qwen-4B: 24\% to 33\%). For Qwen3-30B, the absolute accuracy gain was 5\% (63\% to 68\%), but fine-tuning was critical for robustness.
...
\paragraph{Comparison with Generalist Models:}
Our results contrast with the recent analysis by Wang et al. \cite{wang2024generalist}, which found that off-the-shelf LLMs generally fail to capture the structural complexity of TPC-DS (specifically JOIN paths and predicates), deeming them ``insufficient for practical application''. By achieving \textbf{68\% EMA} via fine-tuning, we demonstrate that the complexity barrier identified by Wang et al. can be overcome through targeted schema alignment, rather than relying solely on model scale.
...
\begin{itemize}
    ...
    \item \textbf{Vector Store}: Baai/bge-m3 for semantic retrieval.
